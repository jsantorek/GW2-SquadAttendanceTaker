...
void SAT::AttendanceWatcher::Start()
{
	...
	if(_isRecording)
	{
		G::APIDefs->Log(ELogLevel_WARNING, ADDON_NAME, "Internal state error - attempting to ::Start() a watcher which IsRecording");
	}
...
void SAT::AttendanceWatcher::Stop()
{
	...
	if(!_isRecording)
	{
		G::APIDefs->Log(ELogLevel_WARNING, ADDON_NAME, "Internal state error - attempting to ::Stop() a watcher which Is(not)Recording");
	}
...
void SAT::AttendanceWatcher::UpdateSquadMembersAttendance()
{
	...
	try
	{
		...
		G::APIDefs->Log(ELogLevel_TRACE, ADDON_NAME, std::to_string(currentUUIDs.size()).append(" squad member(s) acquired").c_str());
	...
	{
		G::APIDefs->Log(ELogLevel_WARNING, ADDON_NAME, "UpdateSquadMembersAttendance encountered unidentifed exception");
		G::APIDefs->Log(ELogLevel_WARNING, ADDON_NAME, e.what());
	}
}
